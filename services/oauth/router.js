...
router.use(session({store: new MemoryStore({reapInterval: 5 * 60 * 1000}), secret: 'abracadabra', resave: true, saveUninitialized: true}));
router.use(myOAP.oauth());
router.use(myOAP.login());
...
router.get('/login', function(req, res, next) {
    ...
    res.end('<html><form method="post" action="/oauth/login"><input type="hidden" name="next" value="/oauth/secret"><input type="text" placeholder="username" name="username"><input type="password" placeholder="password" name="password"><button type="submit">Login</button></form>');
});

router.post('/login', function(req, res, next) {
    req.session.user = req.body.username;

    res.writeHead(303, {Location: req.body.next || '/oauth/secret'});
    res.end();
});

router.get('/logout', function(req, res, next) {
    req.session.destroy(function(err) {
        res.writeHead(303, {Location: '/oauth'});
        res.end();
    });
});
